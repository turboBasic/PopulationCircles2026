...
table and its dataset by, and one payload type per question a
...
5. **Rendering.** Python, from the published document and nothing else, kept out of the Rust search
   ...
   schema version it does not know, a kind it cannot draw, an earth model that is not a sphere, or a
   document naming no dataset — a figure whose credit cannot be read is not drawn;
   ...
   resolves a dataset — by the key the document carries, never by one of its own, and `render` is handed
   the attribution rather than reaching for it. A
